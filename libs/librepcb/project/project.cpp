  ...
  try {
    ...
    if (create) {
      ...
      foreach (const FilePath& fp,
        ...
        mFileSystem->write("resources/fontobene/" % fp.getFilename(),
    ...
    if (!create) {
      SExpression schRoot =
          SExpression::parse(mFileSystem->read("schematics/schematics.lp"));
    ...
    if (!create) {
      SExpression brdRoot =
          SExpression::parse(mFileSystem->read("boards/boards.lp"));
...
bool Project::save(bool toOriginal, QStringList& errors) noexcept {
  ...
  try {
    ...
    mFileSystem->write("schematics/schematics.lp", root.toByteArray());
  } catch (const Exception& e) {
    success = false;
    errors.append(e.getMsg());
  }
  ...
  try {
    ...
    //}
    mFileSystem->write("boards/boards.lp", root.toByteArray());
  } catch (const Exception& e) {
    success = false;
    errors.append(e.getMsg());
  }
