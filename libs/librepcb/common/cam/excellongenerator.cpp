...
void ExcellonGenerator::drill(const Point&          pos,
  ...
  if (!mToolList.contains(dia))
    mToolList.append(dia);
}

void ExcellonGenerator::slot(const Point&          start,
                             const Point&          end,
                             const PositiveLength& width) noexcept {
    mSlotList.insert(width, qMakePair(start, end));
    if (!mToolList.contains(width))
      mToolList.append(width);
}
...
void ExcellonGenerator::printDrills() noexcept {
  for (int i = 0; i < mToolList.count(); i++) {
    ...
    // Circular drills
    for (const Point& pos : mDrillList.values(mToolList[i])) {
      mOutput.append(
          QString("X%1Y%2\n")
    ...
    // Drilled Slots
    for (const QPair<Point, Point> points : mSlotList.values(mToolList[i])) {
        mOutput.append(
            QString("X%1Y%2G85X%3Y%4\n")
                .arg(
                    points.first.getX().toMmString(),
