...
TransactionalDirectory::TransactionalDirectory(
  ...
  : FileSystem(parent), mFileSystem(fs), mPath(dir) {
...
TransactionalDirectory::TransactionalDirectory(TransactionalDirectory& other,
  ...
  : FileSystem(parent),
    ...
    mPath(other.convertPath(subdir)) {
}
...
QString TransactionalDirectory::convertPath(const QString& path) const
    noexcept {
  return mPath.isEmpty() ? path : mPath % "/" % path;
}
